...
class _TimeTrackingPageState extends State<TimeTrackingPage> {
  ...
  /// - Shift must be of a type that allows clocking (Standalone or Child)
  bool get _canManualClockIn {
    if (_activeShift == null) return false;
    if (_isClockedIn) return false;
    if (!_activeShift!.canClockInOut) return false; 
    
    final status = _activeShift!.shiftStatus?.toLowerCase().replaceAll(' ', '_') ?? '';
    // Allow clock-in for scheduled or assigned shifts
  ...
  bool get _canManualClockOut {
    if (!_isClockedIn) return false;
    if (_activeShift == null || !_activeShift!.canClockInOut) return false;
