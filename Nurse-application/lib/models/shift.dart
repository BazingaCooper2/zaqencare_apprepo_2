...
class Shift {
  ...
  // ✅ Shift type classification helpers (UPDATED PER USER RULES)
  bool get isBlock => (shiftMode ?? '').toLowerCase().trim() == 'block';

  bool get isBlockChild => parentBlockId != null;

  bool get isStandalone =>
      (shiftMode ?? '').toLowerCase().trim() == 'individual' &&
      parentBlockId == null;

  bool get isActive => clockIn != null && clockOut == null;

  /// Only individual and child block shifts can be clocked in/out
  bool get canClockInOut => isStandalone || isBlockChild;
